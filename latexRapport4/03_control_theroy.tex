...
To implement LQI, the discrete system matrices ($\textbf{A}$ and $\textbf{B}$) and the penalty matrices ($\textbf{Q}$ and $\textbf{R}$) are expanded to fit this fifth state. The augmented $\textbf{Q}$ matrix receives a new diagonal value specifically to penalize the integral error. The maximum of this value is set to $1.0$ and included in the new $\textbf{Q}$ using Bryson's rule. \newline
