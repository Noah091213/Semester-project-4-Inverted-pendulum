...
Solving the algebraic Riccati equation with these $\textbf{Q}$ and $\textbf{R}$ weights and the statespace (reference to system modelling here!) gives the optimal feedback gain matrix $\textbf{K}$. In the PLC implementation, the control force applied to the cart is calculated simply by multiplying the static gain matrix by the current sensor states:
...
Because the $K_5 x_i$ term continuously builds up control effort as long as a there is an error in the cart position, it guarantees that the cart will settle on its target position, regardless of small physical imperfections or friction.
