...
The system matrices ($\textbf{A}$, $\textbf{B}$, $\textbf{C}$, and $\textbf{D}$ from section \ref{Newton Euler Modelling}) are first discretized using a Zero-Order Hold (ZOH) method. This converts the physical model into a discrete-time representation that aligns with the sampling of the PLC.
