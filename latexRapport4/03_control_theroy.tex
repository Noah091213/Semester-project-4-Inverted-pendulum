...
\comment{Lastly, the Settling Time ($t_s$, $\alpha = 2\%$) is estimated with another second-order approximation, this time on the dominant pole ($p_4$) representing the cart settling. Since the pendulum being upright is dependant on the cart settling.}{Switch sentence order: reasoning, then method}
\begin{equation}
...
To implement LQI, the discrete system matrices and \remove{the }weight matrices are expanded to fit this fifth state. The augmented $\textbf{Q}$ matrix receives a new diagonal value specifically to penalize the integral error. The maximum of this value is set to $1.0$ and included in the new $\textbf{Q}$ using Bryson's rule. \newline
