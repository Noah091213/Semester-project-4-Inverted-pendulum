...
Solving the algebraic Riccati equation with these $Q$ and $R$ weights and the statespace (reference to system modelling here!) gives the optimal feedback gain matrix $K$. In the PLC implementation, the control force applied to the cart is calculated simply by multiplying the static gain matrix by the current sensor states:
...
First, the cart's position error ($e_x$) is calculated by subtracting its current measured position from the target position:
\begin{equation}
...
The control effort ($u$) is then calculated using the energy error, an energy gain constant, and the direction. However, just putting in energy to the pendulum could cause the cart to drift so a small spring force is added to gently center the cart ($-k_x x$):
\begin{equation}
