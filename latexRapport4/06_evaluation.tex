...
For each test, 3 graphs were made. The cart position shows how the cart reacts and moves during the various stages of tests. \replace{Because 0 is in the center of the track, the graphs will always be centered around that, as all units are in the treated units and not in raw encoder units.}{For the cart position graphs, the calculated $x$ position in meters is used, with 0 at the center of the track.} The pendulum angle shows what the angle of the entire pendulum is during the tests. This is shown in degrees measured by the encoder sitting at the base of the pendulum in the carts center. The motor force test is a readout of the force applied by the motor. It is measured in newtons, as the motor drive has a mode specifically for torque, as mentioned in REF[]. 
...
The impulse acts differntly for all 3 controllers specificly because of the system state when the impulse happens. \comment{As the graph shows, all 3 controllers are at a different point in either steady or swinging one or the other way.}{hvaba} Even though they all move to a different spot, they still all move in a very similar way, with a sharp impulse that evens out to find a steady state again.
